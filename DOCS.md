...
| `release-pr-gates` | release PR opened, synchronized, or reopened | Backstops the one PR class `pr-gates` ignores: the `release/vX.Y.Z` version-bump PR (`pr-gates` fires only for `agent/issue-*` heads, so a release PR would merge with zero checks — as PR #384 did). Runs `scripts/version-consistency.mjs` on `release/*` heads only, failing the moment a known version location drifts and naming each disagreeing file next to the expected version. Never fires on `agent/issue-*` PRs, so agent gates are untouched. |
