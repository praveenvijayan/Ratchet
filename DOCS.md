...
Version 5.3.0 · MIT · https://github.com/praveenvijayan/Ratchet
...
scripts/
  ...
  herd-ui-dashboard-columns.test.mjs Regression test for the flipped dashboard columns and page scroll
  herd-ui-combined-cards.test.mjs Regression test for combined character cards, modal log console, and routing roster
  ...
  retroactive-plans.mjs         Flag plans whose work already exists on a branch/PR (check + sync rule)
  ...
  weekly-failure-review.mjs     Weekly report of merged fix/rework PRs, grouped by failure class
  weekly-failure-review.test.mjs Regression test for the weekly failure-pattern review
  ...
  release-pr-gates.test.mjs     Regression test: release/* PRs run the version-consistency check; agent PRs unchanged
.github/workflows/
  ...
  pr-gates.yml                  Run GATES.md gates and PR size check on every PR into main
  release-pr-gates.yml          Run the version-consistency check on release/* PRs
  ratchet-run.yml               OPTIONAL CI runner (off by default)
  red-main-alert.yml            Alert on a workflow failing on main, every 15 min
  weekly-failure-review.yml     Publish the weekly failure-pattern report, every Monday
...
| `review-verdict-sweep` | every 5 min, or manual | Self-heals `review-verdict` misses: GitHub silently skips `pull_request_review` workflows on conflicted PRs (`mergeable_state: dirty`), so a Request Changes review can land without ever flipping its issue. This sweep walks every open PR, reads its latest review, and flips the mapped issue from `state:in-review` to `state:changes-requested` when the latest verdict is `CHANGES_REQUESTED` — the same self-healing pattern `sweep-stale-claims` and `unblock-dependents` use. APPROVED/COMMENTED latest reviews change nothing; a PR mapping to no issue is a logged no-op; an already-changes-requested issue is left untouched (idempotent); a per-PR API failure is logged and never aborts the rest. The decision logic lives in `scripts/review-verdict-sweep.mjs`; this workflow is only its trigger and env. |
| `auto-merge` | every 5 min, or manual | Tiered review: uniform review is both the bottleneck and the gap, so the merge bar follows the plan-declared risk label the issue's PR inherited (`risk:high`, see §5). A PR **without** `risk:high` merges automatically once its checks are green and its latest recorded review verdict is `APPROVED` — no human wait. A PR **with** `risk:high` never merges on the verdict alone: it also needs an `APPROVED` review from a non-bot reviewer, and at least 15 minutes must have elapsed since it became mergeable (measured from the newest check-run completion on the head commit). A `CHANGES_REQUESTED` verdict never merges — that PR belongs to the existing rework path `review-verdict` drives. Every merge is pinned to the head SHA it judged (a racing push makes GitHub reject rather than merge unreviewed code) and leaves a comment recording the risk tier, the check state, and the verdict it acted on. A failed merge leaves the PR un-merged with a comment stating why, marked with the head SHA so that commit is never retried — no silent retry loop. The decision logic lives in `scripts/auto-merge.mjs` (regression-tested off the network by `scripts/auto-merge.test.mjs`); this workflow is only its trigger and env. |
| `red-main-alert` | every 15 min, or manual | Alerts a human when a workflow on `main` goes red — the signal missing behind both multi-week outages. Reads the newest completed run of every workflow on `main` and keeps exactly one open alert per red workflow: an issue labelled `red-main` carrying a `<!-- red-main-alert: <workflow> -->` marker. That issue is the default channel (GitHub alone, no external service) and the ledger that makes alert-on-transition and auto-resolve possible, so it is always written; staying red updates it instead of opening a second alert, and the next green run closes it with a comment. Set the `RED_MAIN_ALERT_WEBHOOK` secret to also deliver the two transitions (red, resolved) to your own channel; a failed delivery is recorded on the alert issue rather than dropped. All logic lives in `scripts/red-main-alert.mjs`; this workflow is only its trigger and env. |
| `weekly-failure-review` | Mondays, or manual | Runs this project's best improvement mechanism on a schedule instead of when someone remembers. It collects the fix/rework PRs merged during the ISO week that just ended — selected by the host's own conventions, `fix_pr_labels` and `fix_pr_title_pattern` in the `## Weekly failure-pattern review` section of `GATES.md`, defaulting to conventional-commit `fix:`/`revert:`/`hotfix:` subjects and the `hotfix` label — groups them by failure class (the conventional-commit scope, else the PR's topic label, else `unclassified`), and publishes one report per week: an issue labelled `failure-review` carrying a `<!-- weekly-failure-review: <week> -->` marker, linking back to the previous week's report and commenting the forward link on it, so the run of reports is walkable in both directions. Every outcome lands in that one channel — a week with no fix-PRs publishes a report saying the scan ran and found nothing, and a failure (unreadable `GATES.md`, an invalid pattern, a GitHub API error) publishes a report titled `(failed)` naming the error — so silence never means "the job broke". **The human's contract is 30 minutes:** read the groups, and file a `plan/*.md` per repeat class worth a rule. The job only surfaces patterns; the planning protocol consumes them. All logic lives in `scripts/weekly-failure-review.mjs`; this workflow is only its trigger and env. |
...
| `sweep-stale-claims` | every 30 min, or manual | Patrols `state:in-progress`, `state:in-review`, and `state:changes-requested`. Freshness is the newest proof of life: a branch commit, a claim event, or a heartbeat issue comment containing `<!-- ratchet-heartbeat -->`. Stale zero-commit claims return to `state:ready` and have the orphan ref deleted; committed branches are kept. In-review issues with no live PR are requeued, while merged PRs whose issue stayed open are moved to `state:blocked` for human cleanup. Changes-requested work is requeued only after the inactivity window. Two knobs control the thresholds: `STALE_HOURS` (default `2`) is the inactivity window for `state:in-progress` and `state:in-review`; `REWORK_GRACE_HOURS` (default matches `STALE_HOURS`) is the separate window for `state:changes-requested`, giving rework extra time after a human review. Both are env vars on the workflow. Every requeue the sweep performs writes the same structured attempt record `ratchet-requeue.mjs` writes (numbered after the records already on the issue); non-requeue outcomes (merged-PR cleanup, criteria-loss draft hold) post a plain comment, and a failed comment post is logged with the issue number but never stalls the sweep — the label transition has already completed. |
| `pr-gates` | any PR opened, synchronized, or reopened | Runs `scripts/run-gates.mjs` as the `gates` job and `scripts/pr-size-check.mjs` as the `size` job on **every PR into `main`**, regardless of head-branch name or which paths it touches (issue #479) — a hand-pushed PR that only changes `scripts/**` or `.github/workflows/**` gets the exact same required checks as an `agent/issue-*` PR. Both jobs judge the PR by the **base branch's** `GATES.md`, not the copy the PR ships (see *Security: gate config is judged from the base branch* below). Branch protection requires both contexts with `enforce_admins` on (`.agents/skills/ratchet-init/SKILL.md` Step 5), so a failing check is unmergeable for everyone, including admins — the only sanctioned way around a red check is the human-triggered `.agents/skills/ratchet-hotfix/SKILL.md` lane, which still runs these same gates and opens a normal PR rather than bypassing them. |
| `release-pr-gates` | release PR opened, synchronized, or reopened | Runs the one check `pr-gates` doesn't carry: `scripts/version-consistency.mjs` on `release/vX.Y.Z` PRs, failing the moment a known version location drifts and naming each disagreeing file next to the expected version. `pr-gates` already covers `gates`+`size` for release PRs like any other; this workflow exists only because that suite has no notion of version-string agreement (a gap that shipped PR #384 with a malformed bump). Never fires on non-release PRs, which carry no version bump for it to check. |
...
`ratchet-requeue.mjs` is not the record's only writer: `sweep-stale-claims`
appends the same marker (through the same encoder) to every requeue it
performs, with the sweep's classification sentence as the reason and no owner,
so abandonment lands in the ledger too. Both sources share the numbering — a
