...
async function check({ gates = "", baseGates, additions, deletions, changedFiles, files, filesRaw }) {
  ...
  // BASE_GATES_FILE is NOT stripped here: run-gates.mjs removes it (and
  // GATES_FILE) from every gate command it spawns, so a suite run as a gate
  // never inherits CI's config in the first place (#494). The per-suite strip
  // PR #492 added is gone; a test that needs the variable passes `baseGates`.
