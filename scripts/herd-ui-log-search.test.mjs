...
import { mkdtempSync, writeFileSync, appendFileSync, rmSync, readFileSync, existsSync } from "node:fs";
...
// it. --------------------------------------------------------------------------
{
  ...
  const planPath = existsSync(join(planDir, "0094-herd-dashboard-log-search.md"))
    ? join(planDir, "0094-herd-dashboard-log-search.md")
    : join(planDir, "done", "0094-herd-dashboard-log-search.md");
  ...
  assert.equal(markers.length, criteriaCount, `one test per #184 acceptance criterion (${criteriaCount})`);
  for (let n = 1; n <= criteriaCount; n++) assert.ok(unique.has(n), `#184 criterion ${n} has a test`);
}
