...
// The cap measures hand-written lines only: generated artifacts are the
// standing excuse of every over-cap PR ("most of it is the lockfile"), and an
// excuse the gate accepts turns the gate into negotiation prose. These defaults
// apply with no host configuration at all, so a host that never writes an
// exclude_paths line still gets a hand-written count. A host extends them via
// exclude_paths and opts out of one explicitly with `!pattern` (#484).
const DEFAULT_EXCLUDE_PATTERNS = [
  // Dependency lockfiles — regenerated by the package manager, never reviewed
  // line by line. Bare filenames, so they match at any depth (monorepos).
  "package-lock.json",
  "pnpm-lock.yaml",
  "yarn.lock",
  "Cargo.lock",
  ...
  "composer.lock",
  "go.sum",
  // Snapshot artifacts — written by the test runner (`--update-snapshots`),
  // reviewed as a diff of behaviour, not as authored lines. Listed twice each
  // because `**/x` is anchored: it needs a leading directory, so the bare form
  // covers the repo-root case.
  "__snapshots__/**",
  "**/__snapshots__/**",
  "*.snap",
  "**/*.snap",
  // Install manifest — generated by the installer to record what it wrote, so
  // every framework update ships a mechanical rewrite of it.
  "ratchet-manifest.json",
...
function readConfig(text) {
  ...
  // Configured excludes *extend* the defaults; they never replace them, so a
  // host cannot silently lose the hand-written-only guarantee by writing one
  // exclude_paths line. The single escape hatch is explicit: `!package-lock.json`
  // drops that one default and says so in the diff (#484).
  const optOuts = new Set();
  const added = [];
  for (const entry of configuredExcludes) {
    if (entry.startsWith("!")) optOuts.add(entry.slice(1).trim());
    else added.push(entry);
  }
  const defaults = DEFAULT_EXCLUDE_PATTERNS.filter((pattern) => !optOuts.has(pattern));

  return {
    maxLines: num("lines", DEFAULT_MAX_LINES),
    maxFiles: num("files", DEFAULT_MAX_FILES),
    excludePatterns: [...new Set([...defaults, ...added])],
  };
}
...
// Criterion 3 (#484): an over-cap verdict has to be self-explanatory, which
// means naming *which* files the count left out, not just how many. Both the
// pass and fail paths print the same list, so a passing PR shows its own
// exemptions too.
const excludedList = size.excluded.length
  ? size.excluded.map((file) => `${file.filename} (${file.pattern})`).join(", ")
  : "none";
const exclusionNote = size.usedFileDetails
  ? ` Exclusions applied: ${excludePatterns.join(", ")}. Excluded as generated (${size.excluded.length} file(s)): ${excludedList}.`
...
if (!overLines && !overFiles) {
  const msg = `PR size OK: ${changedLines} hand-written changed line(s) ≤ ${maxLines}, ${countedFiles} file(s) ≤ ${maxFiles}.${exclusionNote}`;
  notice(msg);
  summary(`### PR size\n\n✅ ${msg}`);
  console.log(msg);
  process.exit(0);
...
const breaches = [
  overLines ? `${changedLines} hand-written changed lines (limit ${maxLines})` : null,
...
const message = [
  ...
  "The cap measures hand-written lines only — the files below were not counted.",
  `Exclusions applied: ${excludePatterns.join(", ")}.`,
  `Excluded as generated: ${excludedList}.`,
].join("\n");
